<script>
	...
	function handleDelete(id) {
		output = output.filter((item) => item.id !== id);
	}
...
<main class="flex flex-col gap-8 w-screen overflow-hidden min-h-screen bg-bgdarkgrey font-patrick text-black m-0 p-0">
			...
			<div class="flex flex-col justify-between items-center bg-white">
				<div class="flex font-bold">
					{#each ['Mtrl Name', 'Mtrl Code', 'Mtrl Unit', 'Vendor', 'Phone#', 'Vendor Email', 'Address', 'Unit Price', 'Status', 'Order Qty', 'Total Amount', 'Date Purchase', 'Delivery Date', 'ETA Date', 'Arrival Date'] as header}
						<div class="border border-gray-300 bg-bgGrey border-none m-0 py-4 2xl:place-content-center sm:w-14 md:w-16 lg:w-20 xl:w-24 2xl:w-28 text-center">
							{header}
						</div>
					{/each}
				</div>
				<div class="bg-white py-2">
					{#each columns as column (column.id)}
						<div class="flex gap-0" id={column.id}>
							...
							/>
							<div class="border-gray-300 sm:w-14 md:w-16 lg:w-20 xl:w-24 2xl:w-28 text-center py-2">
								{computeTotal(column)}
							</div>
							<input
								type="date"
				...
				<div class="w-full flex flex-col justify-end gap-2 bg-white overflow-hidden">
					<div class="flex gap-2 py-2 justify-end">
						<button on:click={addColumn} class="w-24 h-8 text-sm font-bold rounded-lg text-black hover:text-white bg-blue-200 hover:bg-blue-700">Add Column</button>
						<button on:click={handleSubmit} class="w-24 h-8 text-sm font-bold rounded-lg text-black hover:text-white bg-green-200 hover:bg-green-700">Submit</button>
						<button on:click={() => (columns = [])} class="w-24 h-8 text-sm font-bold rounded-lg text-black hover:text-white bg-red-200 hover:bg-red-700">Delete</button>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="flex justify-center">
		<div class="overflow-hidden rounded-lg shadow hidden md:block font-bold bg-gray-700">
			{#if output.length > 0}
				<div class="flex">
					{#each ['Mtrl Name', 'Mtrl Code', 'Mtrl Unit', 'Vendor', 'Phone#', 'Vendor Email', 'Address', 'Unit Price', 'Status', 'Order Qty', 'Total Amount', 'Date Purchase', 'Delivery Date', 'ETA Date', 'Arrival Date', 'Delete'] as header}
					...
					{#each output as item}
						<div key={item.id} class="flex mb-2 items-center">
							...
							<div class="flex-1 text-center">
								<button on:click={() => handleDelete(item.id)} class="h-8 text-sm font-bold rounded-lg text-black hover:text-white bg-red-200 hover:bg-red-700 px-4">Delete</button>
							</div>
						</div>
					{/each}
				</div>
