	...
	<div class="flex flex-col items-center text-center min-h-screen py-10 justify-center">
		<table class="bg-bgLightGray bg-bgGrey rounded-lg divide-y">
			<thead>
				...
				<tr class="grid grid-cols-custom-15 pt-8 justify-items-center">
					...
					<th>
						<button
							class="flex items-center justify-center h-full"
							on:click={() => sortTable('unitprice')}
						>
							<span class="mr-0">Unit price</span>
							<span>{@html sortBy === 'unitprice' ? currentArrow : getArrow('desc')}</span>
						</button>
					</th>
					<th>
						<button
					...
					<th>
						<button
							class="flex items-center justify-center h-full"
							on:click={() => sortTable('totalPrice')}
						>
							<span class="mr-0">Total Price</span>
							<span>{@html sortBy === 'totalPrice' ? currentArrow : getArrow('desc')}</span>
						</button>
					</th>
					<th>
						<button
					...
					<th>
						<button
							class="flex items-center justify-center h-full"
							on:click={() => sortTable('deliverDate')}
						>
							<span class="mr-0">Delivery date</span>
							<span>{@html sortBy === 'deliverDate' ? currentArrow : getArrow('desc')}</span>
						</button>
					</th>
					<th>
						<button
							class="flex items-center justify-center h-full"
							on:click={() => sortTable('etaDate')}
						>
							<span class="mr-0">ETA</span>
							<span>{@html sortBy === 'etaDate' ? currentArrow : getArrow('desc')}</span>
						</button>
					</th>
					<th>
						<button
					...
					<th>
						...
						>
							...
							<span>{@html sortBy === 'vendostatusrAddress' ? currentArrow : getArrow('desc')}</span
							>
						</button>
					</th>
					<th>
					...
					<th>
						...
						>
							<span class="mr-0">Sp Phonenumber</span>
					...
					<th>
						...
						>
							<span class="mr-0">Sp Address </span>
			...
			<tbody class="divide-y border-borderlineGrey">
				{#each displayedInventory as { datePurchase, materialName, materialCode, unit, materialdescription, unitprice, vendor, purchaseqty, deliverDate, etaDate, arrivalDate, vendorPhonenumber, vendorAddress, status }}
					<tr class="grid grid-cols-custom-15 text-base items-center">
						...
						<td class="py-4 px-1">{unitprice}</td>
						<td class="py-4 px-1">{purchaseqty}</td>
						<td class="py-4 px-1">{unitprice * purchaseqty}</td>
						<td class="py-4 px-1">{datePurchase}</td>
						<td class="py-4 px-1">{deliverDate}</td>
						<td class="py-4 px-1">{etaDate}</td>
						<td class="py-4 px-1">{arrivalDate}</td>
						<td class="py-4 px-1">{status}</td>
