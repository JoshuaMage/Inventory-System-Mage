<script>
	...
	async function onSubmit(event) {
	  ...
	  const employeeId = event.target.id.value;  // Get Employee ID from the form
	  const password = event.target.password.value;  // Get password from the form
  
	  try {
		const email = employeeId + '@example.com';  // Construct email using Employee ID
		const userCredential = await signInWithEmailAndPassword(auth, email, password); // Attempt login
